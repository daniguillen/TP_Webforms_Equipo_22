...
namespace Acciones
{
    public class Controller
    {
        ...
        public List<Articulo> ListarArticulo()
        {
            ...
            lector.setearQuery("SELECT a.id,a.Codigo, a.Nombre, a.Descripcion, a.Precio,  m.id as IDMarca, m.Descripcion AS Marca,  c.Id AS IDCategoria,  c.Descripcion ,i.ImagenUrl FROM ARTICULOS a inner JOIN marcas m ON a.IdMarca = m.Id inner JOIN categorias c ON a.idcategoria = c.id inner JOIN IMAGENES i ON a.id = i.IdArticulo order by a.id, i.id;");
            lector.ejecutarLectura();

            int IDArt_anterior = -1;
            Articulo ArtAnterior = new Articulo();
            while (lector.Lector.Read())
            {
                int ArtID = lector.Lector.GetInt32(0);
                if (ArtID != IDArt_anterior)
                {
                    ...
                    Lista.Add(aux);
                    IDArt_anterior = ArtID;
                }
                Lista[Lista.Count - 1].Imagen.Add(lector.Lector.GetString(9));
            }
        ...
        public List<Articulo> Buscar(string buscar)
        {
            ...
            {
                int ArtID = nuevaConexion.Lector.GetInt32(0);
                ...
                articuloList[articuloList.Count - 1].Imagen.Add(nuevaConexion.Lector.GetString(9));
            }
